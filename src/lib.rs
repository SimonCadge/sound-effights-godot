...
impl AudioAnalyser {
    ...
    fn trim_and_analyse_audio(&self, _owner: gdnative::Reference, audio_stream: gdnative::AudioStreamSample) -> Result<Analysis, String> {
        ...
        let audio_data = get_samples_from_audiostream(audio_stream.clone());
        ...
        let is_stereo = audio_stream.is_stereo();
        let bytes_per_sample = match is_stereo {
            true => bytes_per_sample * 2,
            false => bytes_per_sample,
        };
        // Cut out silence at the start and end.
        let resized_audio_stream = resize_audio(audio_stream, start_index * bytes_per_sample, end_index * bytes_per_sample);
        let resized_audio_stream = match resized_audio_stream {
            Ok(new_stream) => new_stream,
            Err(error) => return Err(error),
        };

        return get_analysis(resized_audio_stream);
    }

    #[export]
    fn analyse_audio(&self, _owner: gdnative::Reference, audio_stream: gdnative::AudioStreamSample) -> Result<Analysis, String> {
        return get_analysis(audio_stream);
    }

    #[export]
    fn cut_and_analyse_audio(&self, _owner: gdnative::Reference, audio_stream: gdnative::AudioStreamSample, duration: f32) -> Result<Analysis, String> {
        // Determine how many bytes in the byte array correspond to one f32 sample.
        let audio_format = audio_stream.get_format();
        let bytes_per_sample = match audio_format {
            gdnative::AudioStreamSampleFormat::Format8Bits => 1,
        ...
        let num_samples = (duration * audio_stream.get_mix_rate() as f32).floor() * bytes_per_sample as f32;
        let start_index = (end_index as f32 - num_samples) as usize;

        godot_dbg!(end_index);
        godot_dbg!(num_samples);
        godot_dbg!(start_index);

        let resized_audio_stream = resize_audio(audio_stream, start_index, end_index);
        let resized_audio_stream = match resized_audio_stream {
            Ok(new_stream) => new_stream,
            Err(error) => return Err(error),
        };
...
fn get_analysis(audio_stream: gdnative::AudioStreamSample) -> Result<Analysis, String> {
    // Convert abstract bytes representation into Vector of f32 samples.
    let audio_samples = get_samples_from_audiostream(audio_stream.clone());
...
fn get_samples_from_audiostream(audio_stream: gdnative::AudioStreamSample) -> Result<Vec<f32>, String> {
    ...
    let is_stereo = audio_stream.is_stereo();
    let num_samples: usize = match is_stereo {
        true => (audio_data.len() as usize / bytes_per_sample) / 2,
        false => audio_data.len() as usize / bytes_per_sample,
    };
    ...
    if is_stereo {
        ...
        for index in (0..audio_data.len()).step_by(bytes_per_sample * 2) {
            match bytes_per_sample {
                // Signed 8 bit PCM data ranges from -128 to 127.
                1 => audio_samples[index as usize / (bytes_per_sample * 2)] = ((audio_data.get(index) as f32 / 128.0) + (audio_data.get(index + 1) as f32 / 128.0)) / 2.0,
                // Signed 16 bit PCM data ranges from -32768 to 32767.
                2 => audio_samples[index as usize / (bytes_per_sample * 2)] = ((i16::from_le_bytes([audio_data.get(index), audio_data.get(index + 1)]) as f32 / 32768.0) + (i16::from_le_bytes([audio_data.get(index+2), audio_data.get(index + 3)]) as f32 / 32768.0)) / 2.0,
                _ => {
        ...
        for index in (0..audio_data.len()).step_by(bytes_per_sample) {
            match bytes_per_sample {
                1 => audio_samples[index as usize / bytes_per_sample] = audio_data.get(index) as f32 / 128.0,
                2 => audio_samples[index as usize / bytes_per_sample] = i16::from_le_bytes([audio_data.get(index), audio_data.get(index + 1)]) as f32 / 32768.0,
                _ => {
                    godot_error!("Unexpected number of bytes per sample {}", bytes_per_sample);
                    return Err(format!("Unexpected number of bytes per sample {}", bytes_per_sample));
                },
